...
node* findNode(int id) {
  ...
  for (int i = tree_count - 1; i >= 0; --i) {
    if (tree[i].connection_edge.u == id || tree[i].connection_edge.v == id) {
      return &tree[i];
    }
  }
...
/// @param current_edge The edge that connects the two sets
/// @param parent Array to store the parent of each node
/// @param rank Array to store the rank of each node
...
void unionSet(int u, int v, edge& current_edge, int parent[], int rank[]) {
  ...
  // temporary edge reppresenting the union
  edge union_representation_edge(u, v, current_edge.weight);
  ...
  if (node_u == nullptr && node_v == nullptr) {
    ...
    tree[tree_count] = node(union_representation_edge);
...
std::pair<edge*, int> kruskalAlgo(int nNodes, edge edges[], int nEdges) {
  ...
  for (int i = 0; i < nEdges && minTreeIndex < nNodes; i++) {
    ...
    // union them
    if (u_root != v_root) {
      unionSet(u_root, v_root, edges[i], parent, rank);
